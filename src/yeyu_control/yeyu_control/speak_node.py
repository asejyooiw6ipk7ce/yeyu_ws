...
class DrivingNode(Node):
    ...
    def on_nav_result(self, future):
        ...
        if status == GoalStatus.STATUS_SUCCEEDED:
            ...
            if self.wp_index == 5:   # wp6(최종 도착점) 도착
                ...
                self.notify_tts('전체 코스를 완료했습니다. 성공적으로 주행을 마쳤습니다.')
                return
    ...
    def _process_signal(self, flipped):
        ...
        if color == 'green':
            ...
            if self.green_count >= 3:
                self.green_count = 0
                self.wp_index = 2
                self.notify_tts('신호등이 초록불로 감지되었습니다. 3번 경유점으로 이동합니다.')
    ...
    def _process_speed_sign(self, flipped):
        ...
        if color == 'blue':
            ...
            if self.blue_count >= 3:
                self.blue_count = 0
                self.wp_index = 3
                self.notify_tts('가속표지판이 감지되었습니다. 제한 속도 내로 4번 경유점으로 이동합니다')
                self.mode = DrivingMode.NAV_TO_PARKING
                self.set_speed(0.22)
    ...
    # 타임아웃 또는 재시도 횟수 초과시 여기로 옴 - 주차 실패 TTS 붙일 시 여기 붙이는 게 편함
    ...
    def _on_parking_done(self):
        ...
        self.set_led('END')
        self.notify_tts('주차가 완료되었습니다. 최종 도착점으로 이동합니다.')
