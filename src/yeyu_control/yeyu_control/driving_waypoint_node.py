...
LED_COLOR_MAP = {
    ...
    'TRACING_CRANK':  (128/255, 0.0, 1.0),
    'TRACING_S':      (1.0, 20/255, 147/255),
    'SIGNAL_WAIT': (11/255, 1.0, 11/255),
    'ACCEL_ZONE':  (0.0, 1.0, 1.0),
    'PARKING':     (1.0, 70/255, 0.0),
    'END':         (1.0, 0.0, 0.0),
}
...
class DrivingNode(Node):
    def __init__(self):
        ...
        self.parking_debug_pub = self.create_publisher(
            CompressedImage, '/parking_debug_image/compressed', 10)
        self.s_course_debug_pub = self.create_publisher(
    ...
    def _load_parking_parameters(self):
        ...
        self.CRANK_CREEP_DISTANCE_M = 0.06

        self.S_ROI_TOP_RATIO = 0.6
        self.S_LINE_BLACK_THRESHOLD = 60
        self.S_LINE_PIXEL_MIN = 50
        self.S_LINEAR_SPEED_MAX = 0.07
        self.S_LINEAR_SPEED_MIN = 0.04
        self.S_ANGULAR_GAIN = 0.9
        self.S_ANGULAR_MAX = 0.6
        self.S_OFFSET_DEADBAND = 0.05
        self.S_LINE_LOST_TIMEOUT_SEC = 15.0
        self.S_ARRIVAL_TOLERANCE_M = 0.3
        self.S_OFFSET_JUMP_LIMIT = 0.6
        self.S_BOTTOM_BAND_HEIGHT_RATIO = 0.3
    ...
    def camera_processing_loop(self):
        ...
        if self.mode == DrivingMode.SIGNAL_WAIT:
            ...
            self._process_aruco(flipped, header, image_width=flipped.shape[1], image_height=flipped.shape[0])
        elif self.mode == DrivingMode.TRACING_S:
    ...
    def _detect_s_line_offset(self, cv_image) -> Optional[float]:
        ...
        for c in contours:
            ...
            if cw > w * 0.5:
                reason = 'too_wide'
                debug_candidates.append((x, y, cw, ch, area, 0.0, reason))
                continue


            band_h = max(1, int(ch * self.S_BOTTOM_BAND_HEIGHT_RATIO))
            band_y_start = y + ch - band_h
            ...
            M = cv2.moments(band_mask, binaryImage=True)
            if M['m00'] == 0:
                continue
            cx = x + (M['m10'] / M['m00'])
            ...
            if self.s_last_valid_offset is not None:
                if abs(this_offset - self.s_last_valid_offset) > self.S_OFFSET_JUMP_LIMIT:
                    rejected_reason = 'jump_limit'


            debug_candidates.append((x, y, cw, ch, area, 0.0, rejected_reason))
    ...
    def _draw_s_course_debug_image(self, cv_image, mask, roi_top, cx_full, offset, best_contour=None, band_rect=None, debug_candidates=None):
        ...
        if debug_candidates:
            for (x, y, cw, ch, area, solidity, rejected_reason) in debug_candidates:
                ...
                color = (0, 0, 255) if rejected_reason else (0, 255, 0)
        ...
        if best_contour is not None:
            shifted = best_contour + [0, roi_top]
    ...
    def _check_s_course_arrival(self):
        ...
        if dist <= NEAR_MARGIN:
            ...
            elif dist > self.s_course_min_dist + 0.05:
